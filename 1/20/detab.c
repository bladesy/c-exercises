...
int main(void) {
	...
	while((c = getchar()) != EOF)
		...
		if(c == '\t')
			/* Fill the distance in characters to the next TABSTOP with
			blanks - ensure that each blank is also considered as a
			character and thus incremented. */
			for(charPos = charNum % TABSTOP;
				charPos < TABSTOP; ++charPos, ++charNum)
				putchar(' ');
		/* If the character is not a tab, print it and increment the
